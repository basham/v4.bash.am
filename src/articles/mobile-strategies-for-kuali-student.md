...
While there are numerous [practical considerations for accommodating a variety of impairments](https://webaim.org/articles/userperspective/), the most universal gains are achieved by writing content understandable by people and enhancing it with [semantics understandable by software](https://www.w3.org/TR/aria-in-html/). Practically, content should be well written, structured with appropriate HTML5 tags, and laced with appropriate WAI-ARIA attributes. The interface should be completely [navigable and usable via only the keyboard](https://www.w3.org/WAI/PF/aria-practices/#keyboard). Common interactive components, such as alerts, dialogs, popups, sliders, and tabs, should leverage [best practices](https://www.w3.org/WAI/PF/aria-practices/#aria_ex) when implemented. By doing so, screen readers and other assistive technologies will be able to accurately describe the interface, and the interactions will be translated to the user as the designer intended.

Consequentially, by designing for mobile, we're actually forced to address many concerns that plague those with disabilities, given there are so many [overlaps of design considerations](https://www.w3.org/WAI/mobile/overlap.html). For example, those with low vision often magnify parts of the interface, hiding the rest; the limited screen resolution of mobile encourages us to design for condensed interfaces. Those with deafness cannot respond to audio alerts; vibration or screen notifications may reinforce an audible alert on a mobile device. Those with motor disabilities may have difficulty typing on a keyboard; a tiny mobile keyboard is often strenuous when typing single-handed or long lengths of text. Screen readers read in a linear order; content is typically reformatted into a single column on mobile. Motor or visual disabilities may prevent a mouse from being used; there is no mouse input on mobile. In essence, [mobile becomes a lens to improve interfaces for everyone](https://www.w3.org/WAI/mobile/experiences).
...
* [Element queries](https://ianstormtaylor.com/media-queries-are-a-hack/).
* JavaScript frameworks ([AngularJS](https://angularjs.org), [Ember.js](https://emberjs.com), [Backbone.js](https://backbonejs.org)).
* JavaScript style guide.
* CSS pre-processors ([LESS](http://lesscss.org), [Sass](https://sass-lang.com)) and post-processors ([Autoprefixer](https://github.com/ai/autoprefixer)).
* CSS naming methodology ([BEM](https://en.bem.info/), [OOCSS](http://oocss.org), [SMACSS](https://smacss.com/)).
* CSS frameworks ([Bootstrap](https://getbootstrap.com), [Foundation](https://foundation.zurb.com)).
* Web fonts and iconography.
* Front-end development environments/workflows ([Yeoman](https://yeoman.io/)).
...
* [Web components](https://css-tricks.com/modular-future-web-components/) and future-oriented tools ([Polymer](https://www.polymer-project.org)).
